...
struct ExperimentalCheckInView: View {
    ...
    private var strengthHeader: some View {
        let tier = strengthTierResult.overallTier
        let limitingTier = strengthTierResult.exerciseTiers
            .first(where: { $0.exercise.id == strengthTierResult.limitingExercise.id })?.tier ?? .rookie
    ...
    private func intensityColor(for set: LiftSet) -> Color {
        // Use the e1RM that existed *before* this set was logged (same as original CheckInView)
            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    ...
                    if let ex = selectedExercise {
                        ...
                        let incStr = inc.truncatingRemainder(dividingBy: 1) == 0 ? "\(Int(inc))" : String(format: "%.1f", inc)
                        Text("±\(incStr) lbs · \(userProperties.minReps)-\(userProperties.maxReps) reps")
                            .font(.system(size: 10, weight: .medium))
                            .foregroundStyle(.white.opacity(0.35))
                    }
                    ...
                    Button {
                        ...
                        HStack {
                            VStack(alignment: .leading, spacing: 3) {
                                ...
                                Text("\(Int(card.suggestion.weight)) × \(card.suggestion.reps)")
                            ...
                            VStack(alignment: .trailing, spacing: 3) {
                                Text("e1RM +\(String(format: "%.2f", card.suggestion.delta)) lbs")
                                    .font(.system(size: 13, weight: .semibold))
                                    .foregroundStyle(card.color)
                ...
                if overlayDidIncrease {
                    ...
                    VStack(spacing: 4) {
                        Text("Increased 1RM by")
                            .font(.subheadline)
                            .foregroundStyle(.white)
                        Text("+\(overlayDelta.rounded1().formatted(.number.precision(.fractionLength(2)))) lbs")
    ...
    private func loadDataForExercise(_ exerciseId: UUID) {
        ...
        // Set initial weight/reps based on data
        if current1RM > 0, let ex = selectedExercise {
            let increment = ex.effectiveWeightIncrement
            let suggestions = OneRMCalculator.minimizedSuggestions(current1RM: current1RM, increment: increment)
            let midReps = (userProperties.minReps + userProperties.maxReps) / 2
            if let midSuggestion = suggestions.first(where: { $0.reps == midReps }) {
                weight = midSuggestion.weight
                reps = midSuggestion.reps
    ...
    private func triggerLogSetFlash() {
