...
struct ProteinWidget: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if last7Days.contains(where: { $0.total > 0 }) {
                ...
                EmptyWidgetState(icon: "fork.knife", message: "Log protein to see your trends")
            }
        }
        .padding()
    ...
    private var headerText: String {
        ...
        if let goal = goal {
            return "\(formatted) / \(goal.formatted()) g protein"
        }
        return "\(formatted) g protein"
    }
    ...
    private func proteinAnnotation(for day: DayTotal) -> some View {
        Text("\(day.date.formatted(.dateTime.weekday(.abbreviated))): \(Int(day.total).formatted())g")
